...
export default async function BookPage({
  ...
  if (book.authors?.id) {
    const { data: profile } = await supabase
      .from("profiles")
      .select("whatsapp")
      .eq("id", book.authors.id)
      .single()

    authorWhatsapp = profile?.whatsapp ?? null
  }

  // Incrémentation des vues (sans bloquer le rendu)
  supabase
    .from("books")
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {book.stock_physical > 0
                          ? ${book.stock_physical} en stock
                          : "Épuisé"}
                      </p>
                    </div>
                    <p className="text-2xl font-bold">
                      {book.price_physical.toLocaleString()} FCFA
                    </p>
                  </div>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">
                    ...
                    <p className="text-2xl font-bold">
                      {book.price_pdf.toLocaleString()} FCFA
                    </p>
                  </div>
